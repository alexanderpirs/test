...
class SeatingInviteesController extends \yii\web\Controller
{
...
//                ->innerJoin('agenda_periode_translation','agenda_periode_translation.AGENDA_PERIOD_ID = AGENDA_PERIODE_ID')->andFilterWhere(['=', 'agenda_periode_translation.LANGUAGE_ID', 2]),
    ...
    public function actionSaveManualyInvitee() {
        ...
        if (Yii::$app->request->isPost && $model->load(Yii::$app->request->post()) && $model->validate()) {
            $cirles=isset($_POST['Invitees']['Circle']) && $_POST['Invitees']['Circle']!= null && sizeof($_POST['Invitees']['Circle'])>0 ?$_POST['Invitees']['Circle'] :null;
            $guestname=isset($_POST['Invitees']['guestname']) && $_POST['Invitees']['guestname'] != null && sizeof($_POST['Invitees']['guestname'])>0 ?$_POST['Invitees']['guestname'] :null;
            $placewith=isset($_POST['Invitees']['placewith']) && $_POST['Invitees']['placewith'] != null && sizeof($_POST['Invitees']['placewith'])>0 ?$_POST['Invitees']['placewith'] :null;
  Yii::error('Saving : 1');
            if ($WeddingID != 0) {
                  Yii::error('Saving : 2');
                $model->WEDDING_ID = $WeddingID;
                $model->INVITEE_RESPONSE='N';
                if ($model->save(false)) {
                      Yii::error('Saving : 3');
                    $InviteeID = $model->getPrimaryKey();
                   Yii::error('Saving : 55 : '.print_r($cirles,true) .' empty'); 
                    if ($InviteeID != null) {
                        if ($cirles!=null) {
                            $i = 0;
                            foreach ($cirles as $Circle) {
                                $GuestModel = new \app\models\InviteeGuests();
                                $GuestName = $guestname[$i];
                                $PlaceWith = $placewith[$i];
...
$numberOfInvitees = 0;
  ...
  public function actionAfterAddingManuallyInvitee() {
    ...
    public function actionSearch() {
        ...
        $Invitees = $SearchInvitees->search(Yii::$app->request->post()  );
        Yii::error('Search ');
